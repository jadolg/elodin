...
`429 Too Many Requests` on a connection that stays open. It stays because ending
it charges this server a TLS handshake per refusal and the flooding client
nothing, which made refusing dearer than answering. A connection that is closed
is drained briefly first, since closing a socket with unread data resets it and
the client's kernel would throw away the answers already sent.
