...
validate_answer :: proc(
	...
	for rec in msg.answer {
		...
		status, wildcard, why, signer, tag, algo := validate_rrset(
			v,
			budget,
			rec.name,
			rec.type,
...
validate_rrset :: proc(
	...
	*/
	...
	for sig in sigs {
		...
		*/
		...
		case .Ok:
			return .Secure, expanded, "", sig.signer, sig.key_tag, sig.algorithm
		case .Unsupported:
			unsupported = true
		case .Refused:
			refused = true
	...
	*/
	...
	switch owner_status {
	case .Insecure:
		return .Insecure, false, "unsigned zone", "", 0, 0
	case .Indeterminate:
		return .Indeterminate, false, "chain of trust unavailable", "", 0, 0
	case .Bogus:
		return .Bogus, false, "broken chain of trust", "", 0, 0
	case .Secure:
	}
	...
	*/
	if unsupported {
		return .Bogus, false, "no signature this build can verify", "", 0, 0
	}
	...
	*/
	if refused {
		return .Insecure, false, "algorithm refused by local policy", "", 0, 0
	}
	return .Bogus, false, missing, "", 0, 0
}
