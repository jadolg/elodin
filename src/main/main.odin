...
report :: proc(s: ^server.Server, answers: ^cache.Cache) {
	...
	logx.infof(
		"stats: queries=%d blocked=%d cached=%d forwarded=%d failed=%d dropped=%d refused=%d conn_refused=%d limited=%d truncated=%d secure=%d bogus=%d | cache entries=%d bytes=%d hits=%d misses=%d evictions=%d",
		st.queries,
		st.blocked,
		st.cached,
		st.forwarded,
