...
Parity_Options :: struct {
	...
	upstream:  string,
	// The scenario to run under, by name, or "all" for every one of the mode's.
	// See parity_scenario.odin.
	scenario:  string,
...
Parity_Stats :: struct {
	...
	skipped:    int,
	// Answered with the block response, as the scenario's rules said they
	// would be. See parity_blocking.odin.
	blocked:    int,
	/*
...
run_parity_live :: proc(r: ^Runner, opts: Parity_Options) {
	name := opts.scenario if opts.scenario != "baseline" else "live"
	scenarios := parity_scenarios_named(name, true)
	if scenarios == nil {
		start_case(r, "parity scenario")
		fail(r, "no live scenario is called %q", name)
		end_case(r)
		return
	}
	for s in scenarios {
		run_parity_live_scenario(r, opts, s)
	}
}

@(private = "file")
run_parity_live_scenario :: proc(r: ^Runner, opts: Parity_Options, s: Parity_Scenario) {
	// Heap rather than the arena; see `run_parity_scenario`.
	title := fmt.aprintf("parity against %s: %s - %s (seed %d)", opts.upstream, s.name, s.levers, opts.seed)
	defer delete(title)
	...
	resolver, known := parity_resolver(host)
	if parity_needs_tls(s) && !known {
		skip_case(r, title, "no dot or doh name is known for this resolver; see PARITY_RESOLVERS")
		return
	}
	start_case(r, title)
	defer end_case(r)
	...
	cfg := parity_scenario_config(r, s, udp_port, dot_port, doh_port, tls, ends)
	srv, ok := start_server(
		r,
		Server_Options {
			config = cfg,
	...
	for i in 0 ..< opts.runs {
		parity_one_live(r, &srv, host, port, s, &g, opts, &stats, i)
...
parity_one_live :: proc(
	...
	policy := Parity_Policy {
		mode              = .Live,
		dnssec_validation = s.validate,
		ttl_exact         = false,
		transport         = q.transport,
		client_udp_limit  = parity_client_limit(q, parity_max_udp(s)),
		client_cookies    = s.cookies != .Off,
	}
...
upstream_also_says :: proc(
	...
	*/
	...
	// Without validation the question goes out as the client wrote it, less
	// the same options (`dnssec_upstream_query` is not on that path at all).
...
parity_additional_dropped_case :: proc(r: ^Runner) {
	...
	cfg := parity_scenario_config(
		...
		0,
		false,
		Parity_Endpoints{ports = []int{mock_port}},
	)
	srv, ok := start_server(
		r,
