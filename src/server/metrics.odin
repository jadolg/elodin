		...
		*/
		...
		if err != nil {
			if act.wait > 0 {
				...
				if act.report {
					report_accept_failure("metrics", act.run, err, &ctx.accept_reported)
				}
				time.sleep(act.wait)
			}
			continue
...
serve_metrics :: proc(s: ^Server, l: ^Listeners, socket: net.TCP_Socket, client: net.Endpoint) {
	...
	r := Http_Reader {
		...
		// this is also the bound on the connection - see `Http_Reader.budget`.
		budget = Read_Budget{idle = METRICS_TIMEOUT},
	}
